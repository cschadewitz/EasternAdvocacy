...
class Address extends Model
{
    ...
    public static function parseAddress($street_address) {
        return Address::address($street_address['address'])
            ->city($street_address['city'])
            ->state($street_address['state'])
            ->zip($street_address['zip'])
            ->first();
    ...
    public static function checkRecord($street_address) {//top down for more specificity
        $exists=Address::address($street_address['address'])->first();
        if($exists!=null) {
            $exists=Address::address($street_address['address'])->city($street_address['city'])->first();
            if($exists!=null) {
                $exists=Address::address($street_address['address'])->city($street_address['city'])->state($street_address['state'])->first();
                if($exists!=null) {
                    $exists=Address::address($street_address['address'])->city($street_address['city'])->state($street_address['state'])->zip($street_address['zip'])->first();
                    if($exists!=null) {
                        return true;
                    }
                    else {
